...
| Método | Ruta | Descripción |
|---|---|---|
| GET | `/api/v1/tipos-eleccion` | Catálogo de tipos de elección (array simple, sin paginación) |
| GET | `/api/v1/tipos-eleccion/{codigo}` | Detalle de un tipo de elección |
| GET | `/api/v1/elecciones` | Listado paginado de elecciones con filtros |
| GET | `/api/v1/elecciones/{id}` | Detalle de una elección con tipo expandido |
| GET | `/api/v1/elecciones/{id}/totales-territorio` | Totales territorio de una elección (paginado, con filtros) |
| GET | `/api/v1/elecciones/{id}/totales-territorio/{territorio_id}` | Resultado completo: totales + votos por partido |

### Territorios

| Método | Ruta | Descripción |
|---|---|---|
| GET | `/api/v1/territorios` | Listado paginado de territorios con filtros |
| GET | `/api/v1/territorios/{id}` | Detalle con todos los códigos |
| GET | `/api/v1/territorios/{id}/hijos` | Hijos directos (navegación jerárquica) |

### Partidos

| Método | Ruta | Descripción |
|---|---|---|
| GET | `/api/v1/partidos` | Listado paginado de partidos con filtros |
| GET | `/api/v1/partidos/{id}` | Detalle con agrupación (recode) expandida |
| GET | `/api/v1/partidos-recode` | Listado paginado de agrupaciones de partidos |
| GET | `/api/v1/partidos-recode/{id}` | Detalle de agrupación con lista de partidos asociados |

### Resultados

| Método | Ruta | Descripción |
|---|---|---|
| GET | `/api/v1/resultados/totales-territorio` | Totales territorio filtrables por múltiples criterios |
| GET | `/api/v1/resultados/votos-partido` | Votos por partido filtrables |
| GET | `/api/v1/resultados/combinados` | Votos con partido, territorio y elección expandidos |

## Paginación

Todos los endpoints que devuelven listados (excepto `/api/v1/tipos-eleccion`) usan paginación.
...
#### `/api/v1/partidos-recode`

| Parámetro | Tipo | Ejemplo | Descripción |
|---|---|---|---|
| `agrupacion` | str | `PCE/IU` | Búsqueda parcial por agrupación |

#### `/api/v1/elecciones/{id}/totales-territorio`
...
#### `/api/v1/resultados/totales-territorio`
...
#### `/api/v1/resultados/votos-partido` y `/api/v1/resultados/combinados`
